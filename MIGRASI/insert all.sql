...
FROM 
	(
		SELECT  '00' KDHUBKEL, TRIM(A.NOTAS) NOTAS, A.NIP, A.NO_KPE, A.NAMA, A.JNSKEL, A.TGLLHR, A.TMT_KERJA, A.TMT_TASPEN, A.TGLWAFAT, A.TGLPMK, A.KDKELOMPOK, A.KDJNSPEG, A.KDPANGKAT, A.MASKER, A.KDKEDUDUKAN, A.BUP, A.GAPOK, 
		CASE  WHEN KDSTAPEG > 20 THEN (SELECT MAX(Z.GAPOK97) FROM  MIG_ACB.PERHITUNGAN_THT Z WHERE Z.NOTAS=A.NOTAS )   
		...
		CASE  WHEN KDSTAPEG > 20 THEN (SELECT MAX(Z.THP2) FROM  MIG_ACB.PERHITUNGAN_THT Z WHERE Z.NOTAS=A.NOTAS  ) 
		...
		FROM  
			(
				...
				,(SELECT N.BUP_MAX          FROM  MIG_ACB.KEDUDUKAN_TBL N 	WHERE N.KDDUDUKAN=(SELECT W.KDDUDUKAN FROM  MIG_ACB.MUTASI_HIST W WHERE W.NOTAS=A.NOTAS  AND W.TMT_MUTASI =(SELECT MAX(G.TMT_MUTASI)FROM MIG_ACB.MUTASI_HIST G WHERE G.NOTAS=W.NOTAS ))) as BUP
...
SELECT KDCABANG, TGLKEGIATAN, CALON_STOP, RP_CALON, AKTIF, RP_AKTIF, STOP, RP_STOP, INPUTER, TGLAPPROVAL, TGLSTOP, TGLDAPEM,current_date AS UPDSTAMP, ROW_NUMBER() OVER(PARTITION BY TGLKEGIATAN,KDCABANG ORDER BY TGL_KEGIATAN,TGLDAPEM DESC) AS RN
...
INNER JOIN TSPNMIG.REF_DATI4 E ON  A.KDDATI4=E.KDDATI4 AND A.KDDATI3=E.KDDATI3 AND A.KDDATI2=E.KDDATI2 AND A.KDDATI1=E.KDDATI1 
...
INSERT INTO TSPNMIG.ALAMAT_HIST(TMT_ALM, KDHUBKEL, NOTAS, ALAMAT, KOTA, KDPOS, TELEPON, KDDATI1, KDDATI2, KDDATI3, KDDATI4, NOHP, "EMAIL", UPDSTAMP, INPUTER) SELECT A.TMT_ALM, TRIM(A.KDHUBKEL), TRIM(A.NOTAS), A.ALM_PESERTA, A.KOTA, A.KODEPOS, A.TELEPON, A.KDDATI1, A.KDDATI2, A.KDDATI3, A.KDDATI4,  A.NOHP, "EMAIL",A.UPDSTAMP, A.INPUTER 
FROM MIG_ACB.ALAMAT_HIST A 
INNER JOIN TSPNMIG.KELUARGA B ON TRIM(A.NOTAS) = B.NOTAS AND A.KDHUBKEL = B.KDHUBKEL
INNER JOIN TSPNMIG.REF_DATI4 E ON  A.KDDATI4=E.KDDATI4 AND A.KDDATI3=E.KDDATI3 AND A.KDDATI2=E.KDDATI2 AND A.KDDATI1=E.KDDATI1 
