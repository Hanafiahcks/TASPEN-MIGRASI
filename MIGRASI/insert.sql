...
INSERT INTO TASPEN.PENPOK_HIST(MASKER, JMLANAK, TMT_PENPOK, TMT_EFEKTIF, KDJNSPENERIMA, KDPANGKAT, KDKELOMPOK, KDPRODUK, KDJNSTRANS, KDSIFATPOK, KDHUBKEL, NOTAS, TGLKLAIM, KDJIWA, PCT_PENPOK, PENPOK, JMLISTRI, GAPOK, TMT_AKHIR, UPDSTAMP, INPUTER, TCACAT, TDAHOR) SELECT MASKER, JMLANAK, TMT_PENPOK, TMT_EFEKTIF, KDJNSPENERIMA, TRIM(A.KDPANGKAT), A.KDKELOMPOK, A.KDPRODUK, A.KDJNSTRANS, A.KDSIFATPOK, A.KDHUBKEL, A.NOTAS, TGLKLAIM, A.KDJIWA, A.PCT_PENPOK, PENPOK, JMLISTRI, GAPOK, TMT_AKHIR, A.UPDSTAMP, A.INPUTER,NULL,NULL FROM MIG_ACB.PENPOK_HIST AS A INNER JOIN TASPEN.PENERIMA_SK  AS B ON TRIM(A.KDHUBKEL) = TRIM(B.KDHUBKEL) AND TRIM(A.NOTAS) = TRIM(B.NOTAS);
INSERT INTO TASPEN.TRANS_PENSIUN(KDHUBKEL, NOTAS, TGLPROSES, TGLKLAIM, TMT_TRANS, KDTRANS, NILAI, KDKLAIM, KDJNSTRANS, KDPRODUK, KDKELOMPOK, UPDSTAMP, INPUTER) 
SELECT A.KDHUBKEL, TRIM(A.NOTAS), TGLPROSES, TGLKLAIM, TMT_TRANS, KDTRANS, NILAI, KDKLAIM, KDJNSTRANS, A.KDPRODUK, A.KDKELOMPOK, A.UPDSTAMP, A.INPUTER  FROM MIG_ACB.TRANS_PENSIUN A INNER JOIN TASPEN.PENERIMA_SK  AS B ON TRIM(A.KDHUBKEL) = TRIM(B.KDHUBKEL) AND TRIM(A.NOTAS) = TRIM(B.NOTAS) WHERE NOT EXISTS (SELECT * FROM TASPEN.TRANS_PENSIUN AS C WHERE  A.KDHUBKEL=C.KDHUBKEL AND TRIM(A.NOTAS)=C.NOTAS AND A.TGLPROSES=C.TGLPROSES AND A.TGLKLAIM=C.TGLKLAIM AND A.TMT_TRANS=C.TMT_TRANS AND A.KDTRANS=C.KDTRANS AND A.KDKLAIM=C.KDKLAIM AND A.KDJNSTRANS=C.KDJNSTRANS AND A.KDPRODUK=C.KDPRODUK AND A.KDKELOMPOK=C.KDKELOMPOK) ;
