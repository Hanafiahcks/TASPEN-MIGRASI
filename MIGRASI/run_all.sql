...
DELETE FROM TASPEN.TRANS_PENSIUN WHERE YEAR(TMT_TRANS)>2017;
...
UPSERT 		TASPEN.PENPOK_HIST(MASKER, JMLANAK, TMT_PENPOK, TMT_EFEKTIF, KDJNSPENERIMA, KDPANGKAT, KDKELOMPOK, KDPRODUK, KDJNSTRANS, KDSIFATPOK, KDHUBKEL, NOTAS, TGLKLAIM, KDJIWA, PCT_PENPOK, PENPOK, JMLISTRI, GAPOK, TMT_AKHIR, UPDSTAMP, INPUTER, TCACAT, TDAHOR) SELECT MASKER, JMLANAK, TMT_PENPOK, TMT_EFEKTIF, KDJNSPENERIMA, TRIM(A.KDPANGKAT), A.KDKELOMPOK, A.KDPRODUK, A.KDJNSTRANS, A.KDSIFATPOK, A.KDHUBKEL, TRIM(A.NOTAS), TGLKLAIM, A.KDJIWA, A.PCT_PENPOK, PENPOK, JMLISTRI, GAPOK, TMT_AKHIR, A.UPDSTAMP, A.INPUTER,NULL,NULL FROM MIG_ACB.PENPOK_HIST AS A INNER JOIN TASPEN.PENERIMA_SK  AS B ON TRIM(A.KDHUBKEL) = TRIM(B.KDHUBKEL) AND TRIM(A.NOTAS) = TRIM(B.NOTAS);
...
UPSERT 		TASPEN.TRANS_PENSIUN(KDHUBKEL, NOTAS, TGLPROSES, TGLKLAIM, TMT_TRANS, KDTRANS, NILAI, KDKLAIM, KDJNSTRANS, KDPRODUK, KDKELOMPOK, UPDSTAMP, INPUTER) SELECT A.KDHUBKEL, TRIM(A.NOTAS), TGLPROSES, TGLKLAIM, TMT_TRANS, KDTRANS, NILAI, KDKLAIM, KDJNSTRANS, A.KDPRODUK, A.KDKELOMPOK, A.UPDSTAMP, A.INPUTER  FROM MIG_ACB.TRANS_PENSIUN A INNER JOIN TASPEN.PENERIMA_SK  AS B ON TRIM(A.KDHUBKEL) = TRIM(B.KDHUBKEL) AND TRIM(A.NOTAS) = TRIM(B.NOTAS) 
WHERE YEAR(TMT_TRANS)>2017;
...
INSERT INTO TASPEN.TRS_DAPEM( TGLDAPEM,KDJNSTRANS,NOTAS,KDHUBKEL,KDHUBKEL_WALI,KDJIWA,KDJNSPENSIUN,DESK_JNSPENSIUN,KDCABANG,NMCABANG,PENPOK,TISTRI,TANAK,TP,TKD,TPP,TPAJAK,TBERAS,TCACAT,TTEWAS,TDAHOR,RAPEL,TBULAT,KOTOR,PPAJAK,PASKES,PKPKN,PKASDA,PTASPEN,PSEWA,PASSOS,PALIMENTASI,TGR,POTONGAN,BERSIH,NAMA_PENSIUNAN,TGL_LAHIR_PENSIUNAN,JNS_KELAMIN_PENSIUNAN,NAMA_PENERIMA,TGL_LAHIR_PENERIMA,JNS_KELAMIN_PENERIMA,KDPANGKAT,KDHITUNG,KDSIFATPOK,MASKER,JMLISTRI,NPWP,KDJURUBAYAR,NMJURUBAYAR,NOREK,KDKELBAYAR,NMKELBAYAR,KD_OTENTIKASI,TGL_OTENTIKASI,KD_SETOR,TGL_SETOR,NOSETOR,JNS_OTENTIKASI,FLAG_LOCK,NOTAS_CROSSDEFAULT,JNS_TAGIHAN,TGLREKAM,TGLKEJADIAN,UPDSTAMP,INPUTER)
SELECT 
	AA.TGLDAPEM, AA.KDJNSTRANS, AA.NOTAS, AA.KDHUBKEL, AA.KDHUBKEL_WALI, AA.KDJIWA, AA.KDJNSPENSIUN, AA.DESK_JNSPENSIUN, AA.KDCABANG, AA.NMCABANG, AA.PENPOK, AA.TISTRI, AA.TANAK, AA.TP, AA.TKD, AA.TPP, AA.TPAJAK, AA.TBERAS, AA.TCACAT, AA.TTEWAS, AA.TDAHOR, AA.RAPEL, AA.TBULAT, AA.KOTOR, AA.PPAJAK, AA.PASKES, AA.PKPKN, AA.PKASDA, AA.PTASPEN, AA.PSEWA, AA.PASSOS, AA.PALIMENTASI, AA.TGR, AA.POTONGAN, AA.BERSIH, AA.NAMA_PENSIUNAN, AA.TGL_LAHIR_PENSIUNAN, AA.JNS_KELAMIN_PENSIUNAN, AA.NAMA_PENERIMA, AA.TGL_LAHIR_PENERIMA, AA.JNS_KELAMIN_PENERIMA, AA.KDPANGKAT, AA.KDHITUNG, AA.KDSIFATPOK, AA.MASKER, AA.JMLISTRI, AA.NPWP, AA.KDJURUBAYAR, AA.NMJURUBAYAR, AA.NOREK, AA.KDKELBAYAR, AA.NMKELBAYAR, AA.KD_OTENTIKASI, AA.TGL_OTENTIKASI, AA.KD_SETOR, AA.TGL_SETOR, AA.NOSETOR, AA.JNS_OTENTIKASI, AA.FLAG_LOCK, AA.NOTAS_CROSSDEFAULT, 
	CASE 
	...
	CASE
		WHEN AA.JNS_TAGIHAN = 1 THEN
			(SELECT X.TGLKLAIM FROM MIG_ACB.TAGIHAN_UDW X WHERE TRIM(X.NOTAS) = TRIM(AA.NOTAS) AND X.KDHUBKEL = AA.KDHUBKEL AND X.TGLPROSES = AA.TGLDAPEM AND CAST(X.KDJNSTRANS AS SMALLINT) = AA.KDJNSTRANS) 
		WHEN AA.JNS_TAGIHAN IN (11,22) THEN
			(SELECT X.TGLREKAM FROM MIG_ACB.TAGIHAN_NONUDW X WHERE TRIM(X.NOTAS) = TRIM(AA.NOTAS) AND X.KDHUBKEL = AA.KDHUBKEL AND X.TGLPROSES = AA.TGLDAPEM AND CAST(X.KDJNSTRANS AS SMALLINT) = AA.KDJNSTRANS)
		WHEN AA.JNS_TAGIHAN = 3 THEN
			(SELECT X.TGLREKAM FROM MIG_ACB.TAGIHAN_REKPASIF X WHERE TRIM(X.NOTAS) = TRIM(AA.NOTAS) AND X.KDHUBKEL = AA.KDHUBKEL AND X.TGLPROSES = AA.TGLDAPEM AND CAST(X.KDJNSTRANS AS SMALLINT) = AA.KDJNSTRANS)
	END TGLREKAM,
	CASE
		WHEN AA.JNS_TAGIHAN = 1 THEN
			(SELECT X.TGLWAFAT FROM MIG_ACB.TAGIHAN_UDW X WHERE TRIM(X.NOTAS) = TRIM(AA.NOTAS) AND X.KDHUBKEL = AA.KDHUBKEL AND X.TGLPROSES = AA.TGLDAPEM AND CAST(X.KDJNSTRANS AS SMALLINT) = AA.KDJNSTRANS)
		WHEN AA.JNS_TAGIHAN IN (11,22) THEN
			(SELECT X.TGLKEJADIAN FROM MIG_ACB.TAGIHAN_NONUDW X WHERE TRIM(X.NOTAS) = TRIM(AA.NOTAS) AND X.KDHUBKEL = AA.KDHUBKEL AND X.TGLPROSES = AA.TGLDAPEM AND CAST(X.KDJNSTRANS AS SMALLINT) = AA.KDJNSTRANS)
	END TGLKEJADIAN,
	AA.UPDSTAMP,AA.INPUTER
FROM (
SELECT 
	A.TGLDAPEM,A.KDJNSTRANS,TRIM(A.NOTAS) AS NOTAS,A.KDHUBKEL,COALESCE((SELECT TRIM(X.KDHUBKELWALI) FROM MIG_ACB.PENERIMA_SK X WHERE TRIM(X.NOTAS) = TRIM(A.NOTAS) AND X.KDHUBKEL = A.KDHUBKEL), A.KDHUBKEL) AS KDHUBKEL_WALI,
	A.KDJIWA,A.JENIS AS KDJNSPENSIUN,(SELECT TRIM(X.KETERANGAN) FROM MIG_ACB.KONV_PENSIUN X WHERE TRIM(X.KDJNSPENSIUN) = TRIM(A.JENIS)) AS DESK_JNSPENSIUN,
	A.KODECABANG AS KDCABANG,( SELECT TRIM(X.NMCABANG) FROM MIG_ACB.CABANG_TBL X WHERE TRIM(X.KDCABANG) = TRIM(A.KODECABANG) ) AS NMCABANG,
	A.PENPOK, A.TISTRI, A.TANAK, A.TP, A.TKD, A.TPP, A.TPAJAK, A.TBERAS, A.TCACAT, A.TTEWAS, A.TDAHOR, A.RAPEL, A.TBULAT, A.KOTOR, A.PPAJAK, A.PASKES, A.PKPKN, A.PKASDA, A.PTASPEN, A.PSEWA, A.PASSOS, A.PALIMENTASI, COALESCE(A.TGR,0) AS TGR, A.POTONGAN, A.BERSIH,
	TRIM(A.NAMAPENSIUNAN) AS NAMA_PENSIUNAN,A.TGL_LAHIR_PENSIUNAN,
	(SELECT X.JENKEL FROM MIG_ACB.KELUARGA X WHERE TRIM(X.NOTAS) = TRIM(A.NOTAS) AND X.KDHUBKEL = '00') AS JNS_KELAMIN_PENSIUNAN,
	TRIM(A.NAMA_PENERIMA) AS NAMA_PENERIMA,A.TGL_LAHIR_PENERIMA,
	...
	A.KDPANGKAT,A.KDHITUNG,A.KDSIFATPOK,A.MASKER,A.JMLISTRI,TRIM(A.NPWP) AS NPWP,A.KODEBYR AS KDJURUBAYAR,TRIM(B.NMKANBYR) AS NMJURUBAYAR,TRIM(A.NOREK) AS NOREK,B.KDKELBAYAR,
	(SELECT TRIM(X.NMKELBAYAR) FROM MIG_ACB.KELOMPOK_BAYAR X WHERE X.KDKELBAYAR = B.KDKELBAYAR) AS NMKELBAYAR,
	A.INFOBANK AS KD_OTENTIKASI,A.TGLDIAMBIL AS TGL_OTENTIKASI,A.JNSSETOR AS KD_SETOR,A.TGLSETOR AS TGL_SETOR,
	(SELECT MAX(TRIM(X.NOSETOR)) FROM MIG_ACB.TRANS_DAMU X WHERE TRIM(X.NOTAS) = TRIM(A.NOTAS) AND X.TGLPROSES = A.TGLDAPEM 
	AND CAST(X.KDJNSTRANS AS SMALLINT) = A.KDJNSTRANS AND X.KDSTSDAMU IN (2,3,4) AND X.KDHUBKEL = A.KDHUBKEL ) AS NOSETOR,
	COALESCE(A.KDOTENTIKASI,4) AS JNS_OTENTIKASI,CAST(0 AS SMALLINT) AS FLAG_LOCK,
	...
	CASE
		...
		ELSE 0
	END JNS_TAGIHAN,
	A.UPDSTAMP,TRIM(A.INPUTER) AS INPUTER
...
SELECT 
A.TMT_PREMI, A.TMT_MUTASI AS  BLTHGAJI, TRIM(trailing ' ' from A.NOTAS) NOTAS,A.KDCAB AS KDCABANG, '00'  KDHUBKEL,A.NIPPA NIP,A.NO_KPE,
REPLACE (REPLACE(TRIM(CHAR(160) FROM A.NAMA),'\t',''),'\n','') NAMA,CASE WHEN A.SEX  NOT IN(1,2) THEN 1 ELSE A.SEX END JNSKEL,A.TGLLHR,
A.TMT_KERJA,A.TMT_TASPEN,NULL TGLWAFAT, (SELECT B.TGLPMK FROM MIG_ACB.PESERTA B WHERE B.NOTAS=A.NOTAS) TGLPMK,A.KDKELOMPOK,A.KDJENPEG KDJNSPEG,
TRIM(A.KDPANGKAT) KDPANGKAT,(SELECT Z.MASKER FROM MIG_ACB.MUTASI_HIST Z WHERE Z.NOTAS=A.NOTAS AND Z.TMT_MUTASI=(SELECT MAX(R.TMT_MUTASI) FROM MIG_ACB.MUTASI_HIST R WHERE R.NOTAS=Z.NOTAS AND R.TMT_MUTASI < A.TMT_PREMI)) MASKER,
A.KDDUDUKAN,A.USIA_BUP BUP,A.GAPOK,NULL GAPOK_97,A.KDSTAPEG,
(SELECT Z.PERSENGAPOK FROM MIG_ACB.MUTASI_HIST Z WHERE Z.NOTAS=A.NOTAS AND Z.TMT_MUTASI=(SELECT MAX(R.TMT_MUTASI) FROM MIG_ACB.MUTASI_HIST R WHERE R.NOTAS=Z.NOTAS AND R.TMT_MUTASI < A.TMT_PREMI) ) PERSENGAPOK,
A.TMT_GAPOK, NULL TMT_GAPOK_TBL, '1'||CAST(J_ISTRI AS CHAR(1))||'0'||CAST(J_ANAK  AS CHAR(1)) AS KDJIWA_AKTIF, J_ISTRI AS JMLISTRI, J_ANAK AS JMLANAK,
A.THP NILAI_THP, NULL NILAI_THP_97,
...
FROM  MIG_ACB.PESERTA_CUTOFF AS A INNER JOIN TASPEN.REF_CABANG AS RC ON TRIM(A.KDCAB)=TRIM(RC.KDCABANG)
...
SET 
A.USIA=X.ISI_USIA,
A.USIAMASUK=X.ISI_USIAMASUK,
A.USIA2001=X.ISI_USIA2001,
A.JMLISTRI=X.ISI_JMLISTRI,
A.JMLANAK=X.ISI_JMLANAK,
A.NOURUT=X.ISI_NOURUT
FROM TASPEN.PESERTA_CUTOFF A INNER JOIN (SELECT TANGGAL_CUTOFF,BLTHGAJI,NOTAS,
TASPEN.FC_CARIMASA_TAHUN(TGLLHR,BLTHGAJI) ISI_USIA,
 ROUND(TASPEN.FC_CARIMASA_TAHUN(TGLLHR,TMT_TASPEN)  + TASPEN.FC_HIT_UM(TGLLHR,TMT_TASPEN),2) ISI_USIAMASUK,
 CASE WHEN YEAR(TMT_TASPEN) >  2001 THEN
        USIAMASUK ELSE 
        ROUND(TASPEN.FC_CARIMASA_TAHUN(TGLLHR,CAST('2001-01-01' AS DATE))  + TASPEN.FC_HIT_UM(TGLLHR,CAST('2001-01-01' AS DATE)),2)
        END ISI_USIA2001,
TO_INT(SUBSTRING(KDJIWA_AKTIF,2,1)) ISI_JMLISTRI,
TO_INT(SUBSTRING(KDJIWA_AKTIF,4,1)) ISI_JMLANAK ,
ROW_NUMBER() OVER(partition by notas order by blthgaji,notas)ISI_NOURUT
FROM TASPEN.PESERTA_CUTOFF)  AS X ON A.TANGGAL_CUTOFF=X.TANGGAL_CUTOFF AND A.BLTHGAJI = X.BLTHGAJI AND A.NOTAS = X.NOTAS
;
...
DELETE FROM "TASPEN"."TRANS_PENSIUN" AS A INNER JOIN "TASPEN"."PENERIMA_SK" AS B ON A.KDHUBKEL=B.KDHUBKEL AND A.NOTAS=B.NOTAS WHERE B.NOTAS IS NULL;
DELETE FROM "TASPEN"."PESERTA_CUTOFF" AS A INNER JOIN "TASPEN"."REF_CABANG" AS B ON A.KDCABANG=B.KDCABANG WHERE B.KDCABANG IS NULL;
DELETE FROM "TASPEN"."PENPOK_HIST" AS A INNER JOIN "TASPEN"."PENERIMA_SK" AS B ON A.KDHUBKEL=B.KDHUBKEL AND A.NOTAS=B.NOTAS WHERE B.NOTAS IS NULL;
